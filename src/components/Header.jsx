    ...
    <>
      <header className="bg-white text-gray-800 fixed top-0 inset-x-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          {/* Logo */}
          <Link to="/" className="text-2xl font-bold text-orange-500">Admin Module</Link>
          ...
          <nav className="hidden md:flex space-x-6">
            {navLinks.map(link => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `px-3 py-2 font-medium transition ${isActive ? 'text-orange-500' : 'text-gray-700 hover:text-orange-500'}`
                }
              >
                {link.name}
              </NavLink>
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 z-50 animate-fade-in">
                  ...
                  <ul className="max-h-64 overflow-y-auto divide-y">
                    {notifications.map(note => (
                      <li
                        key={note.id}
                        className={`${note.read ? 'bg-gray-50 text-gray-500' : 'bg-white text-gray-800'} px-4 py-3 hover:bg-gray-100 transition-colors`}
                      >
                        <div className="flex justify-between items-center">
                          <Link
                            to={`/task/${note.taskId}/subtasks`}
                            className="flex-1 hover:underline"
                            onClick={() => { markAsRead(note.id); setShowNotifications(false); }}
                          >
                            {note.message}
                          </Link>
                          {!note.read && (
            ...
            <nav className="flex flex-col p-4 space-y-3">
              {navLinks.map(link => (
                <NavLink
                  ...
                  onClick={() => setMobileOpen(false)}
                  className={({ isActive }) =>
                    `px-3 py-2 rounded-md transition ${isActive ? 'bg-gray-200 text-orange-500' : 'text-gray-700 hover:text-orange-500'}`
                  }
                >
                  {link.name}
                </NavLink>
